...
async def flush_resolved(links: list) -> None:
    """Write/append newly resolved entry to resolved_manifest.txt immediately."""
...
async def main() -> None:
    ...
    log(f"[INFO] Direct Link Resolver (Slow) — Concurrency: {CONCURRENCY} | Safe: {CONCURRENCY_SAFE}")

    # Clear resolved_manifest on fresh run
    ...
    async with async_playwright() as playwright:
        ...
        log("[OK] Stage 2 (Slow) finished — resolved_manifest.txt is ready.")
